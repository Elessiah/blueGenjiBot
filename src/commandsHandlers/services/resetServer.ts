...
async function _resetServer(client: Client,
    ...
    try {
        ...
        return {success: false, message: "An error occurred while trying to reset server : " + (err as TypeError).message};
    }
}
...
async function resetServer(client: Client,
    ...
    const ret: status = await _resetServer(client, interaction.guildId);
    if (ret.success) {
